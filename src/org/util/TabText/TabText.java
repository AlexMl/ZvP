...
public class TabText {
     ...
     * 
     * @param chars
     *        a string with the chars to be added (careful with unicode or ansi chars, do some tests before)
     * @param charsWidth
     ...
     * 
     * @param page
     *        desired page number (0 = all in one), considering preconfigured page height
     * @param monospace
     *        true if fonts are fixed width (for server console) or false if variable with (for chat area)
     * @return desired formatted, tabbed page
     */
    public String getPage(int page, boolean monospace) {
	
	// get bounds if user wants pages
	int fromLine = (--page) * this.chatHeight;
	...
	for (int linePos = fromLine; linePos < toLine; ++linePos) {
	    ...
	    fields = this.lines[linePos].split("`", -1);
	    line = "";
	    lineLen = 0;
	    lineLen2 = 0;
		...
		tab = (monospace) ? this.tabs[fieldPos] : this.tabs[fieldPos] * 6;
     ...
     * @param mono
     *        true if lenght will be in chars (for console) or false if will be in pixels (for chat area)
     ...
     * 
     * @param keys
     *        a list of column numbers criteria where 1 represents first columnn and so, negative numbers means descending order
     */
	...
	for (int i = 0; i < this.lines.length; ++i) {
	    ...
	    // iterate fields
	    fields = this.lines[i].replaceAll("§.", "").split("`", -1);
	    line = "";
		...
		try {
		    ...
		    for (int k = field.length(); k < CHATLENGTH; ++k) {
			field2 += " ";
		    }
		    field2 += field;
