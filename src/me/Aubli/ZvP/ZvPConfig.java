...
import me.Aubli.ZvP.Game.Score.ScoreBenefit;
...
public class ZvPConfig {
	...
	insertComments.addComment("premium", "Add additional money multipliers on permission basis. Example:##" + "premium:##The name of the benefit#  normal:##The permission node for the benefit#    permission: zvp.play##The factor, zombie-funds are multiplied with#    positiv: 1.8##The factor, death fees are multiplied with#    negativ: 0.6");
    ...
    public static ScoreBenefit[] getBenefits() {
	List<ScoreBenefit> benefits = new ArrayList<>();

	for (Entry<String, Object> entry : getConfig().getConfigurationSection("premium").getValues(false).entrySet()) {
	    String permission = getConfig().getString("premium." + entry.getKey() + ".permission");
	    double positiv = getConfig().getDouble("premium." + entry.getKey() + ".positiv");
	    double negativ = getConfig().getDouble("premium." + entry.getKey() + ".negativ");
	    benefits.add(new ScoreBenefit(permission, positiv, negativ));
	}

	ScoreBenefit[] beArray = new ScoreBenefit[benefits.size()];
	for (ScoreBenefit be : benefits) {
